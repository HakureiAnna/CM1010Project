function ClimateChange() {
  ...
  this.addSettings = function(parent) {    
    ...
    self.minYear = self.data.getNum(0, 'year');
    self.maxYear = self.data.getNum(self.data.getRowCount() - 1, 'year');
    console.log(self.minYear, self.maxYear);
    ...
    label.for = 'sldStartYear';
    label.innerHTML = 'Start Year:';
    container.appendChild(label);    
    var sldStart = document.createElement('input');
    sldStart.type = 'range';
    sldStart.min = self.minYear;
    sldStart.max = self.maxYear;
    sldStart.step = 1;
    sldStart.value = self.minYear;
    container.appendChild(sldStart);
    self.startSlider = sldStart;

    var container = document.createElement('div');
    container.className = 'form-group';
    parent.appendChild(container);
    var label = document.createElement('label');
    label.for = 'sldEndYear';
    label.innerHTML = 'End Year:';
    container.appendChild(label);    
    var sldEnd = document.createElement('input');
    sldEnd.type = 'range';
    sldEnd.min = self.minYear;
    sldEnd.max = self.maxYear;
    sldEnd.step = 1;
    sldEnd.value = self.minYear +1;
    container.appendChild(sldEnd);
    self.endSlider = sldEnd;
  }
